...
int spiht_code_ra4(long int *image, unsigned char *stream, long int *outputsize, struct coder_param_struct coder_param)
{
...
// int maxres=2;
#ifdef RES_SCAL
int maxresspec=6;//WARNING WARNING WARNING
int maxresspat=6;
// int maxresspec=4;
// int maxresspat=4;
int maxres=maxresspec*maxresspat;	
int resspat=0;
int resspec=0;
#else
int maxres=NRES;
...
// dist = eval_dist_grp(iloc, jloc, kloc, image, imageprop, (coder_param.maxquant[blockind])+1);
...
#ifdef RES_RATE
//      add_to_rddata(&((datablock[blockind].rddata)[res]), *(datablock[blockind].streamlast)*8+*(datablock[blockind].count)-ratebefore, dist);
#else
...
// #ifndef INPLACE
// 				insert_el_after(LIS[res],el,lastLIScurrentthres);
// #else
...
int spiht_decode_ra4(long int *image, unsigned char *stream, long int *outputsize, struct coder_param_struct coder_param)
{
...
// int maxquant=MAXQUANT_CONST;
// int maxquant=(int) *maxquantvalue;
// int maxquant=(int) coder_param.maxquant[0];
...
err=desinterleavingblocks(datablock, nblock, stream, *outputsize, coder_param.nlayer);//carefull for the bit counting later...
...
for (res=0; res<(coder_param.maxres[blockind]); res++){	
// 	if (res<3){
// 		minquant =0; //WARNING valid ONLY if encoding was full !!!
// 	}else{
// 		minquant=(coder_param.minquant[blockind]);
// 	}
...
for (thres_ind=(coder_param.maxquant[blockind]); thres_ind >= (coder_param.minquant[blockind]); thres_ind--){	
	if (thres_ind>10){
		maxres =NRES; //WARNING valid ONLY if encoding was full !!!
	}else{
...
// 			add_to_stream(stream, count, (int) bitsig, streamlast);
			...
			image[trans_pixel(current_child->pixel,imageprop)] += threshold; //we don't want to add it UNLESS we have the sign
			if (bitsig == 0){
