...
//decode
spiht_decode_c(imageout, stream, outputsize);
printf("Fin decodage\n");
...
void list_free(struct list_struct * list){
	struct list_el * previous=list->first;
	struct list_el * current_el=list->first;
	while (current_el != NULL){
		previous=current_el;
		current_el=current_el->next;
		free(previous);
...
void insert_el(struct list_struct * list, struct list_el * el)
{
	if (list->last == NULL){/* The list is still empty*/ 
		...
		list->last->next = el;
		list->last = el;
		list->last->next=NULL;/*Et oui, si on ne met à la fin, il ne faut plus rien derrière...*/
	}
};
