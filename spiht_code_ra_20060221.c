...
// #endif
// #ifdef CASEB
// struct list_el * lastLSC[MAXQUANT_CONST];
// struct list_el * lastLIC[MAXQUANT_CONST];
// #endif
...
rate_i = 200.; //min rate to remember
rate_e = 3000000.; //max rate to remember
...
datablock[blockind].rddata.r[i]=0; //faster to use separate loops for memory access ? done by compiler ?
...
//distortion completed once only, the rest is just value tracking
...
// #ifdef CASEA
//remember what was the last one (only in case a. of p 25 bit plane scaling priority)
